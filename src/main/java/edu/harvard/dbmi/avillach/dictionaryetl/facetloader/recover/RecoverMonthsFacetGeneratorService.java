...
public class RecoverMonthsFacetGeneratorService {
    ...
    public GenerateRecoverMonthsResponse generate(GenerateRecoverMonthsRequest req) {
        // Category defaults (now curated facets by default)
        String categoryName = defaultStr(req.categoryName, "Consortium_Curated_Facets");
        String categoryDisplay = defaultStr(req.categoryDisplay, categoryName.replace('_', ' '));
        String categoryDescription = defaultStr(req.categoryDescription, "Consortium Curated Facets Description");

        // Parent facet defaults
        String parentName = defaultStr(req.parentFacetName, "Recover Adult");
        String parentDisplay = defaultStr(req.parentFacetDisplay, "RECOVER Adult");
        String parentDescription = defaultStr(req.parentFacetDescription, "Recover adult parent facet.");

        // RECOVER Adult scoping expressions (copied into children)
        String studyId = defaultStr(req.studyId, "phs003436");
        String adultNodeRegex = defaultStr(req.adultNodeRegex, "(?i)RECOVER_Adult(_v\\d+)?$");
        ...
        FacetCategoryWrapper wrapper = buildWrapper(
                categoryName, categoryDisplay, categoryDescription,
                parentName, parentDisplay, parentDescription,
    ...
    private FacetCategoryWrapper buildWrapper(
        ...
        p0.exactly = studyId; p0.node = 0;
        FacetExpressionDTO p1 = new FacetExpressionDTO();
        p1.regex = adultNodeRegex; p1.node = 1;
        ...
        List<FacetDTO> children = months.stream().map(m -> {
            ...
            e1.regex = "(?i)^(inf|noninf)$"; e1.node = -2;
            FacetExpressionDTO e2 = new FacetExpressionDTO();
            e2.exactly = String.valueOf(m); e2.node = -1;
