...
public interface ConsentRepository extends JpaRepository<ConsentModel, Long> {
    ...
    @Modifying
    @Transactional
    @Query(value = "DELETE FROM dict.consent WHERE dataset_id IN (SELECT dataset_id FROM dict.dataset WHERE ref = :ref)", nativeQuery = true)
    int deleteByDatasetRef(@Param("ref") String ref);
}
